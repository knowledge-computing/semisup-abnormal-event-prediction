...
def train_tgconv_ngat_ae(data_loader, model, device, args):
    
    mse_loss_func = nn.MSELoss(reduction='sum').to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (data, unsup_data) in enumerate(data_loader):
            ...
            _, node_rec = model(data)
            _, unsup_node_rec = model(unsup_data)
        ...
        print(f'| Epoch: {epoch}/{self.n_epochs} | Train Time: {epoch_train_time:.3f}s '
              f'| Train Loss: {epoch_loss / num_samples:.6f} |')
        logging.info(f'| Epoch: {epoch}/{self.n_epochs} | Train Time: {epoch_train_time:.3f}s '
                     f'| Train Loss: {epoch_loss / num_samples:.6f} |')

    train_time = time.time() - start_time
    print('Pretraining Time: {:.3f}s'.format(train_time))
    logging.info('Pretraining Time: {:.3f}s'.format(train_time))
