...
class TGConvNGATCCTrainer(BaseTrainer):
...
#                     extend_centers = center_loss_func.centers.detach().unsqueeze(0).repeat(num_emb, 1, 1)
                ...
                ass_prob, pseudo_y = _assignment(unsup_graph_emb)
                unsup_center_loss = center_loss_func(unsup_graph_emb, pseudo_y)
...
#                 kl_loss = kl_loss_function(assignment.log(), target) / assignment.shape[0]
                ...
                loss += center_loss + unsup_center_loss
                #loss += kl_loss
...
#                 unsup_center_losses += unsup_center_loss.item()
